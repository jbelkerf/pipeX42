...
/*   Updated: 2025/01/16 16:11:50 by jbelkerf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_2(t_pip *pip, char *file_path, int option)
{
	...
	out_file = pip->argv[pip->argc - 1];
	if (option == 2)
	{
		pip->infd = open(file_path, O_RDONLY, 0777);
		pip->cmd_start = 2;
		out_file = pip->argv[pip->argc - 1];
		pip->outfd = open(out_file, O_WRONLY | O_APPEND | O_CREAT, 0666);
	}
	else if (option == 1)
	{
		pip->infd = open(file_path, O_RDONLY, 0777);
		pip->cmd_start = 1;
		pip->outfd = open(out_file, O_WRONLY | O_TRUNC | O_CREAT, 0666);
	}
	if (pip->infd == -1)
		error(file_path);
	if (pip->outfd == -1)
